...
class LR_ARD(object):
    ...
    def fit(self, z, y, z_tst=None, y_tst=None, hyper=None, maxit=30, pruning=8e-2):
        self.z = z
        self.t = y
        self.z_tst = z_tst
        self.t_tst = y_tst
        ...
        self.myModel1.fit(
            x_train, y_train_1, x_test, y_tst_1, prune=0, maxit=self.maxit
        )
        pesos1 = self.myModel1.return_w()[:-1, :].ravel()
        maximo1 = np.max(np.abs(pesos1))
        self.pesos1 = np.where(np.abs(pesos1) < maximo1 * self.pruning, 0.0, pesos1)

        self.myModel2 = DBL_class.LR_ARD()
        self.myModel2.fit(
            x_train, y_train_2, x_test, y_tst_2, prune=0, maxit=self.maxit
        )
        pesos2 = self.myModel2.return_w()[:-1, :].ravel()
        maximo2 = np.max(np.abs(pesos2))
        self.pesos2 = np.where(np.abs(pesos2) < maximo2 * self.pruning, 0.0, pesos2)

        self.myModel3 = DBL_class.LR_ARD()
        self.myModel3.fit(
            x_train, y_train_3, x_test, y_tst_3, prune=0, maxit=self.maxit
        )
        pesos3 = self.myModel3.return_w()[:-1, :].ravel()
        maximo3 = np.max(np.abs(pesos3))
        self.pesos3 = np.where(np.abs(pesos3) < maximo3 * self.pruning, 0.0, pesos3)
